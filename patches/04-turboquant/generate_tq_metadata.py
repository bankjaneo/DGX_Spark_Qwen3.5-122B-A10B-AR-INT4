#!/usr/bin/env python3
"""Generate default TurboQuant metadata (turboquant_kv.json) for Qwen3.5-397B-A17B.
...
# Qwen3.5-397B-A17B: 60 layers, but only every 4th uses full attention
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate TurboQuant metadata (turboquant_kv.json) for Qwen3.5-397B-A17B.",
...
""",
    ...
    output_path = (
        Path(args.output_path) if args.output_path else model_dir / "turboquant_kv.json"
    )
    ...
    if is_q8k:
        print(
            f"  V outlier dims:  {value_indices[0][0]}..{value_indices[0][-1]}  ({outlier_count}/{HEAD_SIZE})"
        )
        ...
        print(
            f"  K outlier dims:  {key_indices[0][0]}..{key_indices[0][-1]}  ({outlier_count}/{HEAD_SIZE})"
        )
        print(
            f"  V outlier dims:  {value_indices[0][0]}..{value_indices[0][-1]}  ({outlier_count}/{HEAD_SIZE})  ← shifted"
        )
        print()
